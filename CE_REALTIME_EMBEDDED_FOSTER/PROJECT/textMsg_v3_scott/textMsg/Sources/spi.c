...
void spi_RAM_config(char options) {
    PTS_PTS7 = 0;//&= (~PTS_PTS7_MASK); // CS enabled  
    sci0put(SPI_WRSR_INST);
    sci0put(options);
    PTS_PTS7 = 1;//= PTS_PTS7_MASK;   // CS disabled
    return;
}
...
void spi_RAM_write_string(unsigned int address, char* string) {
    ...
    PTS_PTS7 = 0;//&= (~PTS_PTS7_MASK);           // CS enabled (pull port S, pin 7 low)
    ...
    sci0put('\0');                          /* Since the loop stops when the null char is reached,
                                               ...
                                               the end of the loop */
    PTS_PTS7 = 1;//= PTS_PTS7_MASK;               // CS disabled (pull port S, pin 7 high)
...
void spi_RAM_read_string(unsigned int address, char* string) {
    ...
    /**string = spi_RAM_read(address);
    while (*string != '\0' || counter < 158)
    {
        string++;
        address++;
        counter++;
